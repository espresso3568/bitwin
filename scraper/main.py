...
def update_dates_manifest():
    os.makedirs("docs", exist_ok=True)
    dates = set()
    for fname in os.listdir("docs"):
        m = re.match(r"data_(\d{8})\.json$", fname)
        if m:
...
def scrape_iii():
    ...
    try:
        ...
        for row in rows:
            ...
            href = (
                f"https://bid.iii.org.tw/bid/list/{href_attr.lstrip('/')}"
                if "bid_no=" in href_attr
                else f"https://bid.iii.org.tw/bid/list/bid_new_list.aspx?bid_no={case_no}"
            )
            found_dates = []
            for td in cols:
                m_list = re.findall(
                    r"(\d{2,4}[/\-\.]\d{1,2}[/\-\.]\d{1,2})", td.text.strip()
                )
...
def scrape_itri():
    ...
    try:
        ...
        resp = session.post(
            relay_url,
            headers=headers,
            json={"BidDocStatus": "", "IseBid": "", "currentPage": 1, "PageSize": 100},
            timeout=TIMEOUT,
        )
            ...
            bids.append(
                {
                    ...
                    "預算金額": str(
                        bid_info.get("Budget", bid_info.get("BudgetAmount", ""))
                    ),
...
def scrape_sinica():
    ...
    for i in range(SINICA_LOOKBACK_DAYS):
        ...
        try:
            r = requests.get(
                f"https://srp.sinica.edu.tw/InviteBids?searchPubTime={date}",
                headers=HEADERS,
                timeout=TIMEOUT,
            )
            ...
            for t in soup.find_all("table"):
                if "標" in t.get_text() or "採購" in t.get_text():
                    table = t
                    break
            if not table:
                continue
            ...
            for row in table.find_all("tr")[1:]:
                ...
                raw = {
                    headers[idx]: col.text.strip()
                    for idx, col in enumerate(cols)
                    if idx < len(headers)
                }
                bid = {"來源": "中研院", "標題連結": href, "公告日": date}
                for k, v in raw.items():
                    if "案號" in k or "標號" in k:
                        bid["案號"] = v
                    elif "名稱" in k:
                        bid["標題"] = v
                    elif "預算" in k:
                ...
                bids.append(bid)
            if bids:
                all_dfs.append(pd.DataFrame(bids))
        except:
            continue
    return (
        pd.concat(all_dfs).drop_duplicates(subset=["案號"], keep="first")
        if all_dfs
        else pd.DataFrame()
    )
...
def main():
    ...
    old_df = (
        pd.read_csv(latest_path, dtype={"案號": str})
    ...
    if not final_df.empty:
        # 合併策略：日期完整度優先
        final_df["tmp_score"] = final_df.apply(
            lambda x: (1 if x["公告日"] != "-" and x["公告日"] != "" else 0)
            + (1 if x["截止日"] != "-" and x["截止日"] != "" else 0),
            axis=1,
        )
        final_df = final_df.sort_values(
            by=["案號", "tmp_score"], ascending=[True, False]
        )
        final_df = (
            final_df.drop_duplicates(subset=["案號"], keep="first")
            .drop(columns=["tmp_score"])
            .fillna("")
        )
        ...
        def is_recent(d):
            try:
                ...
                cutoff = (datetime.now(TW_TZ) - timedelta(days=2)).replace(
    ...
    res_json = {
        ...
        "total": len(final_df),
        "sources": {
            s: len(final_df[final_df["來源"] == s]) for s in final_df["來源"].unique()
        }
        if not final_df.empty
        else {},
